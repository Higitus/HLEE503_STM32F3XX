...
int main(void)
{
	/* STEP 0 * Init & Variables */
	  Address = ADDR_FLASH_PAGE_101;//choix arbitraire
       ...
       if (HAL_FLASHEx_Erase(&EraseInitStruct, &PageError) != HAL_OK)
	    {
	      while (1)
	      {
	        BSP_LED_On(LED5);/*s'allume donc en cas d'erreur*/
	      }
	    }
	...
	HAL_FLASH_Program(TYPEPROGRAM_WORD, Address, nb_dbl_clic);
